...
### AWS status
- Lambda **CÓ code (đã fix theo audit 2026-07-08, chưa deploy — chờ Bedrock Agent)**: `agent-bff` (auth in-Lambda Supabase-validate ES256, sessionId namespace theo user chống hijack, cap input, try/except+CORS), `agent-action-handler` (parse cả `requestBody` không chỉ `parameters` — bug P0 cũ mất title/description; fail-closed userId; whitelist field chống mass-assignment; delete trả 404 khi không xóa được; cap độ dài; apiPath passthrough). Runbook end-to-end + Guardrail + hardened instructions: **`aws/bedrock/DEPLOY-cmd.md`**.
