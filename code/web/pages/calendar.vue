<template>
  <div class="animate-in">
    <h1 class="mb-6 text-xl font-bold text-neutral-950 dark:text-dark-text">History</h1>

    <div class="mb-6 flex items-center gap-4">
      <button @click="prevMonth" class="btn-ghost text-sm">Prev</button>
      <h2 class="text-base font-semibold text-neutral-950 dark:text-dark-text">{{ monthLabel }}</h2>
    ...
    <div class="card mb-6 overflow-x-auto">
      <div class="grid grid-cols-7 gap-1 min-w-[600px]">
        <div v-for="d in dayHeaders" :key="d" class="p-1 text-center text-2xs font-medium text-neutral-950/30 dark:text-white/20">{{ d }}</div>
        <div v-for="i in firstDayOffset" :key="'e' + i" />
        <div v-for="day in daysInMonth" :key="day" class="aspect-square rounded p-1 text-center text-2xs transition-all hover:ring-2 hover:ring-interactive-blue/50" :class="getDayClass(day)" :title="getDayTitle(day)">{{ day }}</div>
      </div>
      <div class="mt-3 flex items-center justify-end gap-1 text-2xs text-neutral-950/25 dark:text-white/15">
        Less <div class="flex gap-0.5"><div v-for="l in 5" :key="l" class="h-3 w-3 rounded" :class="levelClass(l)" /></div> More
      </div>
    </div>

    <div class="card !p-0 overflow-hidden">
      <div class="px-5 py-3 border-b border-neutral-200 dark:border-dark-border"><h2 class="text-sm font-semibold text-neutral-950 dark:text-dark-text">Sessions This Month</h2></div>
      <div v-if="monthSessions.length === 0" class="py-8 text-center text-sm text-neutral-950/20 dark:text-white/15">No sessions recorded this month.</div>
        ...
        <table class="table-base">
          ...
          <tbody>
            <tr v-for="s in monthSessions" :key="s.id">
              ...
              <td class="whitespace-nowrap text-neutral-950/50 dark:text-white/30">{{ Math.round(s.durationPlanned / 60) }}m</td>
              <td class="whitespace-nowrap font-medium text-neutral-950 dark:text-dark-text">{{ Math.round((s.durationActual ?? s.durationPlanned) / 60) }}m</td>
              <td class="whitespace-nowrap"><EmotionBadge v-if="s.emotionLabel" :label="s.emotionLabel" /><span v-else class="text-neutral-950/15 dark:text-white/10">--</span></td>
              <td class="max-w-[200px] truncate text-neutral-950/30 dark:text-white/20">{{ s.journalText || '--' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
function getDayClass(day: number) { const m = dailyMinutes.value[day] || 0; if (m === 0) return 'bg-neutral-150 dark:bg-dark-surface text-neutral-950/15 dark:text-white/10'; return `${levelClass(getLevel(m))} text-white font-medium` }
