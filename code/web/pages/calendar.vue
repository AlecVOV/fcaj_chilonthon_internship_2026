<template>
  <div class="animate-in">
    <h1 class="mb-6 font-display text-display-sm text-ink dark:text-on-dark">{{ t('calendar.title') }}</h1>

    <div class="mb-6 flex items-center gap-4">
      <button @click="prevMonth" class="btn-ghost text-sm">{{ t('calendar.prev') }}</button>
      <h2 class="font-display text-lg text-ink dark:text-on-dark">{{ monthLabel }}</h2>
      <button @click="nextMonth" class="btn-ghost text-sm">{{ t('calendar.next') }}</button>
    </div>
    ...
    <div class="card mb-6 overflow-x-auto">
      ...
      <div class="mt-3 flex items-center justify-end gap-1 text-2xs text-ink-soft dark:text-on-dark-soft/70">
        {{ t('calendar.less') }} <div class="flex gap-0.5"><div v-for="l in 5" :key="l" class="h-3 w-3 rounded-sm" :class="levelClass(l)" /></div> {{ t('calendar.more') }}
      </div>
    </div>

    <div class="card !p-0 overflow-hidden">
      <div class="px-5 py-3 border-b border-hairline dark:border-hairline-dark"><h2 class="text-sm font-medium text-ink dark:text-on-dark">{{ t('calendar.sessionsThisMonth') }}</h2></div>
      <div v-if="monthSessions.length === 0" class="py-8 text-center text-sm text-ink-soft dark:text-on-dark-soft/70">{{ t('calendar.noSessionsThisMonth') }}</div>
      <div v-else class="overflow-x-auto">
        <table class="table-base">
          <thead><tr><th>{{ t('calendar.tableDate') }}</th><th>{{ t('calendar.tablePlanned') }}</th><th>{{ t('calendar.tableActual') }}</th><th>{{ t('calendar.tableMood') }}</th><th>{{ t('calendar.tableJournal') }}</th></tr></thead>
          <tbody>
...
function getDayTitle(day: number) { const m = dailyMinutes.value[day] || 0; return m > 0 ? t('calendar.minOnDate', { min: m, date: `${currentMonth.value.format('MMM')} ${day}` }) : t('calendar.noFocusSessionsTooltip') }
