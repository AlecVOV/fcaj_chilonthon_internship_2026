<template>
  <div class="animate-in">
    <div class="mb-6">
      ...
      <p class="mt-1.5 text-sm text-ink-body dark:text-on-dark-soft">{{ t('admin.overview.adminPanelSubtitle') }}</p>
    </div>

    <div class="mb-6 flex border-b border-hairline dark:border-hairline-dark">
      <NuxtLink to="/admin" class="tab" :class="$route.path === '/admin' ? 'tab-active' : ''">{{ t('admin.tabOverview') }}</NuxtLink>
      <NuxtLink to="/admin/users" class="tab" :class="$route.path.startsWith('/admin/users') ? 'tab-active' : ''">{{ t('admin.tabUsers') }}</NuxtLink>
      ...
      <NuxtLink to="/admin/analytics" class="tab" :class="$route.path.startsWith('/admin/analytics') ? 'tab-active' : ''">{{ t('admin.tabAnalytics') }}</NuxtLink>
    </div>
    ...
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <NuxtLink to="/admin/users" class="card group hover:bg-surface-card dark:hover:bg-surface-dark-elevated transition-colors">
        ...
        <h2 class="font-display text-lg text-ink dark:text-on-dark mb-1">{{ t('admin.overview.userManagementTitle') }}</h2>
        <p class="text-sm text-ink-body dark:text-on-dark-soft">{{ t('admin.overview.userManagementDesc') }}</p>
      </NuxtLink>
      ...
      <NuxtLink to="/admin/media" class="card group hover:bg-surface-card dark:hover:bg-surface-dark-elevated transition-colors">
        ...
        <h2 class="font-display text-lg text-ink dark:text-on-dark mb-1">{{ t('admin.overview.mediaLibraryTitle') }}</h2>
        <p class="text-sm text-ink-body dark:text-on-dark-soft">{{ t('admin.overview.mediaLibraryDesc') }}</p>
      </NuxtLink>
      ...
      <NuxtLink to="/admin/ambient" class="card group hover:bg-surface-card dark:hover:bg-surface-dark-elevated transition-colors">
        ...
        <h2 class="font-display text-lg text-ink dark:text-on-dark mb-1">{{ t('admin.overview.ambientTitle') }}</h2>
        <p class="text-sm text-ink-body dark:text-on-dark-soft">{{ t('admin.overview.ambientDesc') }}</p>
      </NuxtLink>
      ...
      <NuxtLink to="/admin/feedback" class="card group relative hover:bg-surface-card dark:hover:bg-surface-dark-elevated transition-colors">
        <span v-if="newFeedbackCount > 0" class="absolute right-4 top-4 rounded-full bg-coral px-2 py-0.5 text-2xs font-medium text-white dark:bg-coral-dark">{{ t('admin.overview.newBadge', { n: newFeedbackCount }) }}</span>
      ...
      <NuxtLink to="/admin/analytics" class="card group hover:bg-surface-card dark:hover:bg-surface-dark-elevated transition-colors">
        <div class="mb-3 flex h-10 w-10 items-center justify-center rounded-lg bg-accent-teal/20 text-accent-teal dark:bg-accent-teal/30 dark:text-accent-teal">
          <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" /></svg>
        </div>
        <h2 class="font-display text-lg text-ink dark:text-on-dark mb-1">{{ t('admin.overview.analyticsTitle') }}</h2>
        <p class="text-sm text-ink-body dark:text-on-dark-soft">{{ t('admin.overview.analyticsDesc') }}</p>
      </NuxtLink>
      ...
      <div class="card">
        ...
        <h2 class="font-display text-lg text-ink dark:text-on-dark mb-1">{{ t('admin.overview.systemHealthTitle') }}</h2>
        <p class="text-sm text-ink-body dark:text-on-dark-soft">{{ t('admin.overview.systemHealthDesc') }}</p>
      </div>
    </div>
  </div>
</template>
