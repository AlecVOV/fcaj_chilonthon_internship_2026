<template>
  <div class="animate-in">
    <div class="mb-6 flex items-center justify-between">
      <h1 class="font-display text-display-sm text-ink dark:text-on-dark">Admin Feedback</h1>
    </div>
    ...
    <div class="mb-6 flex border-b border-hairline dark:border-hairline-dark">
      ...
      <NuxtLink to="/admin/feedback" class="tab tab-active">Feedback</NuxtLink>
    </div>
    ...
    <div v-if="loadError" class="mb-6 rounded-lg border border-error/30 bg-error/5 p-4">
      ...
      <p class="mt-1 text-xs text-ink-muted dark:text-on-dark-soft">
        Thường do chưa chạy migration <code>00017_feedback.sql</code>.
      </p>
    </div>

    <div class="grid gap-4 sm:grid-cols-3 mb-6">
      <div class="card"><p class="text-2xs font-medium uppercase tracking-wider text-ink-muted dark:text-on-dark-soft">Total</p><p class="mt-2 font-display text-2xl text-ink dark:text-on-dark">{{ items.length }}</p></div>
      <div class="card"><p class="text-2xs font-medium uppercase tracking-wider text-ink-muted dark:text-on-dark-soft">New</p><p class="mt-2 font-display text-2xl text-ink dark:text-on-dark">{{ counts.new }}</p></div>
      <div class="card"><p class="text-2xs font-medium uppercase tracking-wider text-ink-muted dark:text-on-dark-soft">Resolved</p><p class="mt-2 font-display text-2xl text-ink dark:text-on-dark">{{ counts.resolved }}</p></div>
    </div>

    <div class="card !p-0 overflow-hidden">
      <div class="px-5 py-3 border-b border-hairline dark:border-hairline-dark"><h2 class="text-sm font-medium text-ink dark:text-on-dark">All Feedback</h2></div>
      <div v-if="isLoading" class="py-8 text-center text-sm text-ink-soft dark:text-on-dark-soft/70">Loading…</div>
      <div v-else-if="items.length === 0" class="py-8 text-center text-sm text-ink-soft dark:text-on-dark-soft/70">No feedback yet.</div>
      <div v-else class="overflow-x-auto">
        <table class="table-base">
          <thead><tr><th>From</th><th>Message</th><th>Status</th><th>Sent</th><th>Actions</th></tr></thead>
          <tbody>
            <tr v-for="f in items" :key="f.id">
              ...
              <td>
                <div class="flex gap-1">
                  <button v-if="f.status !== 'read'" @click="setStatus(f, 'read')" class="rounded-md px-1.5 py-0.5 text-sm text-primary hover:bg-primary/10">Mark read</button>
                  <button v-if="f.status !== 'resolved'" @click="setStatus(f, 'resolved')" class="rounded-md px-1.5 py-0.5 text-sm text-success dark:text-success hover:bg-success/10">Resolve</button>
                </div>
              </td>
            </tr>
          </tbody>
...
async function refresh() {
  ...
  try {
    ...
    loadError.value = e?.message || 'Failed to load feedback.'
  } finally {
