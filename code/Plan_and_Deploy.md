...
>
> **Update (2026-07-10):** `report-generator` Lambda **đã bị bỏ khỏi kế hoạch hoàn toàn** (quyết
> định scope, không phải "chưa làm" — folder `aws/lambdas/report-generator/` đã xóa, route
> `/report` đã gỡ khỏi `api-gateway/openapi.yaml`). Export report giờ **thuần client-side**
> (`useReportExport.ts` render Markdown + tải file ngay trên browser, không còn gọi API/Lambda
> nào). Mọi mục A4/D3/L9/`report-generator` bên dưới liên quan Lambda này (S3, SES, EventBridge,
> Pandoc/PDF) **không cần làm nữa** — giữ nguyên trong tài liệu làm lịch sử/tham khảo, đã đánh dấu
> tại chỗ. **Lưu ý:** đây là gap so với RFP gốc (RFP yêu cầu tự động gửi email báo cáo mỗi đêm —
> xem `docs/project-assessment.md` update note đầu file để biết chi tiết ảnh hưởng đánh giá) —
...
[AWS]
  ...
  [report-generator ĐÃ BỎ KHỎI KẾ HOẠCH 2026-07-10 — export report giờ thuần client-side,
...
| A4 | ~~**Lambda `report-generator`** (`{md_content,user_id,date}`→`{markdown_url,pdf_url,message}`), S3 upload + Pandoc/PDF + SES~~ — **🚫 bỏ khỏi kế hoạch 2026-07-10**, xem update note đầu file | L | S3 bucket, SES verify, EventBridge, chốt contract |
...
| D3 | ~~**EventBridge** rule `cron(59 16 * * ? *)` → `report-generator` + `lambda:AddPermission` cho `events.amazonaws.com` + DLQ~~ — **🚫 bỏ khỏi kế hoạch 2026-07-10** (phụ thuộc A4, cũng đã bỏ) | M | A4 |
...
**Giai đoạn 4 — Lambdas**
11. Viết & deploy `emotion-detector`, `rag-recommender`, `admin-vectorizer/embed` (A1–A3). **A4 `report-generator` đã bỏ khỏi kế hoạch, bỏ qua.** Lưu ý: `deploy.sh` hiện chỉ `update-function-code` — lần đầu phải `create-function`.
...
### 9.4 Report
- **Luồng (đổi 2026-07-10):** ~~`ExportReportButton.vue` → `useReportExport` → `POST {API}/report {md_content,user_id,date}` → `report-generator` → upload Markdown lên S3 (+ optional Pandoc→PDF) → SES email → `{markdown_url,pdf_url,message}`. Thiếu API ⇒ fallback tải `.md` client.~~ **Thực tế hiện tại:** nút ⬇ Report trong Worklog History (`profile.vue`) → `useReportExport.downloadReport(date)` → render Markdown + tải `.md` **thuần client**, không gọi API/Lambda nào — không còn khái niệm fallback.
...
```
*Caption (lỗi thời — xem update note đầu file):* ~~Report — `ExportReportButton.vue` (đặt ở `/dashboard`) → `useReportExport` → `/report` → `report-generator` upload S3 + gửi SES; Pandoc→PDF cần đóng gói qua layer/container (Lambda runtime không có sẵn); EventBridge cron chạy hằng đêm cho toàn user; thiếu API thì tải `.md` client.~~ Diagram trên vẫn vẽ node `report-generator`/`REP` (chưa gỡ khỏi Mermaid) nhưng **thực tế đã bỏ khỏi kế hoạch 2026-07-10**: nút ⬇ Report trong Worklog History → `useReportExport` render + tải `.md` thuần client, không còn nhánh Lambda/S3/SES/EventBridge nào.
...
| L9 | ~~`report-generator` (A4)~~ | ~~`{md_content,user_id,date}`~~ | **🚫 bỏ khỏi kế hoạch 2026-07-10** — không có Lambda để test, report chạy client-side |
